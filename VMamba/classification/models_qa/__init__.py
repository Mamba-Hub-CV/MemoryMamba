...
# still on developing...
def build_vssm_model(config, memory_size, is_pretrain=False):
...
def build_vssm_models_(cfg="vssm_tiny", ckpt=True, only_backbone=False, with_norm=True,
    CFGS = dict(
        vssm_tiny=dict(
            ...
            ckpt='./vssm_tiny_0230_ckpt_epoch_262.pth',
        ),
        vssm_small=dict(
            model=dict(
                depths=[2, 2, 15, 2], 
            ...
            ckpt='./vssm_small_0229_ckpt_epoch_222.pth',
        ),
        vssm_base=dict(
            model=dict(
                depths=[2, 2, 15, 2], 
            ...
            ckpt='./vssm_base_0229_ckpt_epoch_237.pth',
        ),
    ),
    ckpt_key="model",
    **kwargs):
