...
class GlobalMemoryNetwork(nn.Module):
    def __init__(self, channels, memory_size):
        super(GlobalMemoryNetwork, self).__init__()
        self.global_feature_extractor = nn.Conv2d(channels, memory_size, kernel_size=1)

        self.memory_keys = nn.Parameter(torch.randn(memory_size, memory_size))
        self.memory_values = nn.Parameter(torch.randn(memory_size, channels))

    def forward(self, x):
        global_features = F.avg_pool2d(self.global_feature_extractor(x), (x.size(2), x.size(3)))
        global_features = global_features.view(global_features.size(0), -1)  # B x memory_size
        
        attention_scores = F.softmax(torch.matmul(global_features, self.memory_keys), dim=1)
        memory_response = torch.matmul(attention_scores, self.memory_values)

        memory_response = memory_response.view(memory_response.size(0), -1, 1, 1).expand(-1, -1, x.size(2), x.size(3))
        
        return memory_response, attention_scores
...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, x, x1, x2):
        similarity1 = F.cosine_similarity(F.avg_pool2d(x, (x.size(2), x.size(3))), F.avg_pool2d(x1, (x1.size(2), x1.size(3))), dim=1).unsqueeze(-1)
        similarity2 = F.cosine_similarity(F.avg_pool2d(x, (x.size(2), x.size(3))), F.avg_pool2d(x2, (x2.size(2), x2.size(3))), dim=1).unsqueeze(-1)

        x1 = torch.cat([x, x1], dim=1)
        x2 = torch.cat([x, x2], dim=1)

        x1 = self.fc(x1)*similarity1
        x2 = self.fc(x2)*similarity2
        return x1, x2
...
class SS2D(nn.Module):
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        if d_conv > 1:
            ...
            self.c_memory = GlobalMemoryNetwork(d_inner, memory_size)
            self.f_memory = GlobalMemoryNetwork(d_inner, memory_size)
            self.bef_proj = DepthwiseSeparableConv(d_inner, 192)
            self.aft_proj = DepthwiseSeparableConv(d_inner, 192)
            self.fusion = FeatureFusionModule(d_inner*2, d_inner)
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if not self.channel_first:
            x = x.permute(0, 3, 1, 2).contiguous()
        if with_dconv:
            bef_x = self.bef_proj(x)
            x = self.conv2d(x) # (b, d, h, w)
            x1, attn1 = self.c_memory(x)
            x2, _ = self.f_memory(x)
            x1, x2 = self.fusion(x, x1, x2)
            x = x + 0.1*x1 + 0.1*x2
            aft_x = self.aft_proj(x2)
        x = self.act(x)
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        def info_nce_loss(features1, features2, temperature=0.5):
            ...
            labels = torch.cat([torch.arange(batch_size, 2*batch_size), torch.arange(batch_size)]).to(features1.device)
        ...
        if self.ssm_branch:
            if self.post_norm:
                ...
                tmp, attn1, bef_x, aft_x = self.op(input)
                x = input + self.drop_path(self.norm(tmp))
            else:
                # x = input + self.drop_path(self.op(self.norm(input)))
                tmp, attn1, bef_x, aft_x = self.op(self.norm(input))
                x = input + self.drop_path(tmp)
            # print(x.shape, bef_x.shape, aft_x.shape)
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
        return x, attn1, info_nce_loss(bef_x, aft_x)
...
class VSSLayer(nn.Module):
    ...
    def forward(self, x):
        atten1 = []  # List to accumulate additional outputs
        atten2 = []  # List to accumulate additional outputs
        for block in self.blocks:
            x, attn1, attn2 = block(x)
            atten1.append(attn1)
            atten2.append(attn2)

        # Optionally, process the additional_outputs list here if needed
        x = self.downsample(x)
        return x, atten1, atten2  # Return the main output and all additional outputs
...
class VSSM(nn.Module):
    ...
    def _make_layer(
        ...
        **kwargs,
        ...
        return VSSLayer(blocks, downsample)      
    ...
    def forward(self, x: torch.Tensor):
        ...
        for layer in self.layers:
            x, attn1, loss = layer(x)
            attns1.append(attn1)
            loss_list += loss
        x = self.classifier(x)
        loss_list = [0.05 * loss_list[i] for i in range(len(loss_list))]

        return x, attns1, torch.mean(torch.clamp(torch.stack(loss_list), 0, 3))
